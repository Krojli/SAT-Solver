class Formula():
    def __ne__(self, other):
        #pokličemo z self!=other ali self<>other
...
class Not(Formula):
    ...
    def simplify(self):
        ...
        #* da vsak element v seznamu gleda posebaj
        #če brez *, potem je seznam ena spremenljivka
...
class And(Formula):
    ...
    def evaluate(self, values):
        return all([y.evaluate(values) for y in self.terms])
...
class Or(Formula):
    ...
    def evaluate(self,values):
        return any([y.evaluate(values) for y in self.terms])
